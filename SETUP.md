...
확인 주기는 `HolidayUpdateService.checkInterval`(**하루**), ETag 조건부 요청이라 같으면 0바이트, 새 버전이면 묻지 않고 바로 적용합니다.
...
앱 진입 시 확인에는 하루 throttle이 걸려 있지만, **MY → 공휴일 데이터 → [지금 확인]**
...
> 달력에 표시할 공휴일 목록 파일을 앱을 켤 때 하루에 한 번 GitHub에서 읽기 전용으로 받아 옵니다.
