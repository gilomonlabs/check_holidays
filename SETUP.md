...
갱신을 **시작하는 곳**은 `HolidayRefreshManager` 입니다 — **앱에 들어올 때와 백그라운드에서 돌아올 때**
(달력이 열릴 때도 부르지만 같은 throttle 이라 중복 접속은 없습니다. 2026-09-11 전까지는 달력이 열릴 때만이어서
달력이 없는 스킨에선 한 번도 확인이 안 됐습니다).
확인 주기는 `HolidayUpdateService.checkInterval`(**3일**), ETag 조건부 요청이라 같으면 0바이트, 새 버전이면 묻지 않고 바로 적용합니다.
...
1. 앱을 설치하고 켠다(**앱에 들어올 때** 갱신 확인이 시작됩니다)
2. **내역 → 달력**으로 이동 — 달력이 **즉시** 그려지는지 확인(네트워크를 기다려 멈추면 안 됩니다)
...
앱 진입 시 확인에는 3일 throttle이 걸려 있지만, **MY → 공휴일 데이터 → [지금 확인]**
...
> 달력에 표시할 공휴일 목록 파일을 앱을 켤 때 3일에 한 번 GitHub에서 읽기 전용으로 받아 옵니다.
> 이때 계정·기기 식별자·기록 등 어떤 정보도 보내지 않으며, 바뀐 게 없으면 내용 없이 응답만 받습니다.

(모니모아의 실제 방침 문구는 `public/privacy.html` 4항 "정보를 맡기는 곳"의 GitHub 항목입니다.)
...
| 특정 날짜만 취소 | `removed` 에 넣고 `version` +1 — 앱 내장 날짜·고정 양력까지 이 방법으로 지워진다(README "지우려면"). `dates` 에서 빼는 것만으로는 이 파일로 들어온 날짜만 빠진다 |
